module Auditfiles
  module Xaf20
    class Base
      # Parses the found value to one we like.
      class LedgerType
        def self.parse(string)
          case string
          when 'B', 'BAL', 'Balans', 'A', 'BAS', 'Blns', 'Activa', 'Passiva', 'Debiteur',
            'Crediteur'
            'B'
      ...
      # Map the header of the auditfile
      class Header
      ...
      # Map the transactionlines of the auditfile
      ...
      # Map the transactions of the auditfile
      class Transaction
        include SaxStream::Mapper
